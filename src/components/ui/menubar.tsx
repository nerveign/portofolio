    ...
    <DropdownMenu modal={false} open={open} onOpenChange={setOpen}>
      ...
      <DropdownMenuContent sideOffset={4} className="w-48 lg:hidden gap-2 flex flex-col z-50 backdrop-blur-lg bg-white/80 dark:bg-[#181818]/90">
        <DropdownMenuItem className="text-base cursor-pointer">
          <ItemList route="/" icon={<Home className="size-5 text-white" />} heading="Home" text="Back to home" />
        </DropdownMenuItem>
        <DropdownMenuItem className="text-base cursor-pointer">
          <ItemList route="/works" icon={<Briefcase className="size-5 text-white" />} heading="Works" text="List of projects" />
        </DropdownMenuItem>
        <DropdownMenuItem className="text-base cursor-pointer">
          <ItemList route="/contact" icon={<Phone className="size-5 text-white" />} heading="Contact" text="Contact detail" />
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
